  ...
  methods: {
    ...
    createItem (field) {
      ...
      if (type === 'multi-line-text') {
         ...
         * 数字
         */
        return (<a-input-number {...this.getDisabled(field)} rows={3} v-decorator={[`values.${field.id}`, { initialValue: field.value ? field.value : 0, rules: this.getRules(field) }]} placeholder={`请输入${field.name}`}/>)
      } else {
        /**
         * 默认文本
         */
        return (<a-input {...this.getDisabled(field)} rows={3} v-decorator={[`values.${field.id}`, { initialValue: field.value, rules: this.getRules(field) }]} placeholder={`请输入${field.name}`}/>)
      }
    },
